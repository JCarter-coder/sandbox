function maximumScore(nums: number[], k: number): number {
    ...
    const getPrimes = (limit: number): Set<number> => {
    ...
    let nextDominant: number[] = new Array(N).fill(N);
    let prevDominant: number[] = new Array(N).fill(-1);

    const decreasingPrimeScoreStack: number[] = new Array();
    ...
    const numOfSubArrays: number[] = new Array(N);
    for (let i = 0; i < N; i++) {
    ...
    while (k > 0) {
        let element: number[] = sortedArray[processingIndex++];
        let num = element[0];
        let index = element[1];
        //console.log(`num: ${num}`);
